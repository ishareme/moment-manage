<template>
    <div class="reading-list">
        ...
        <el-table align="center" v-loading="loading"  :data="readingArticleList.docs" tooltip-effect="dark" style="width: 100%">
            ...
            <el-table-column v-if="role === 'admin'"  prop="enable" label="是否有效" show-overflow-tooltip width="100">
                <template slot-scope="scope">
                    <el-switch
                            ...
                            @change="changeReadingArticleInfoEnable(scope.$index, readingArticleList.docs)">
                    </el-switch>
                </template>
            </el-table-column>
            <el-table-column v-if="role === 'admin'"  prop="is_top" label="是否置顶" show-overflow-tooltip width="100">
                <template slot-scope="scope">
                    <el-switch
                            ...
                            active-color="#13ce66"
                            inactive-color="#ff4949"
                            @change="changeReadingArticleInfoIsTop(scope.$index, readingArticleList.docs)">
                    </el-switch>
                </template>
            </el-table-column>
            <el-table-column label="操作" width="100">
                <template slot-scope="scope">
                    <el-button class="edit-btn" size="mini" type="primary" plain round @click="editPost(scope.$index, readingArticleList.docs)"><i class="iconfont icon-caozuo-bianji"></i></el-button>
                    <el-button size="mini" v-if="scope.row.status === 0 && role === 'admin'" type="primary" plain  @click="toHandleStatus(scope.$index, readingArticleList.docs, 4)">待发布</el-button>
                    <el-button size="mini" v-if="scope.row.status === 2 && role === 'admin'" type="primary" plain  @click="toHandleStatus(scope.$index, readingArticleList.docs, 4)">待发布</el-button>
                    ...
                    <el-dropdown size="mini" v-if="scope.row.status === 1 && role === 'admin'" split-button type="info">
                        ...
                        <el-dropdown-menu slot="dropdown">
                            <el-dropdown-item ><span @click="toHandleStatus(scope.$index, readingArticleList.docs, 2)">审核通过</span></el-dropdown-item>
                            <el-dropdown-item><span @click="toHandleStatus(scope.$index, readingArticleList.docs, 3)">审核失败</span></el-dropdown-item>
                        </el-dropdown-menu>
                    </el-dropdown>
                    <el-button size="mini" v-if="scope.row.status === 0 && role === 'author'" type="primary" plain  @click="toHandleStatus(scope.$index, readingArticleList.docs, 1)">发布审核</el-button>
        ...
        <div class="pagination">
            <div v-if="readingArticleList.pageInfo">
                <el-pagination
                        @size-change="handleSizeChange"
                        @current-change="handleCurrentChange"
                        :current-page.sync="readingArticleList.pageInfo.current"
                        :page-size="readingArticleList.pageInfo.pageSize"
                        layout="total, prev, pager, next"
                        :total="readingArticleList.pageInfo.totalItems"
                >
                </el-pagination>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapActions({
                getReadingArticleList: types.GET_READINGARTICLELIST,
            }),
            changeReadingArticleInfoEnable(index, rows){
                let rowData = rows[index];
                this.axios.updateReadingArticleInfo({
                    _id: rowData._id,
            ...
            changeReadingArticleInfoIsTop(index, rows){
                let rowData = rows[index];
                this.axios.updateReadingArticleInfo({
                    _id: rowData._id,
