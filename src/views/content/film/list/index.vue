<template>
    <div class="film-list">
        ...
        <el-table align="center" v-loading="loading"  :data="filmArticleList.docs" tooltip-effect="dark" style="width: 100%">
            ...
            <el-table-column prop="film_info.name" label="影视名称" show-overflow-tooltip sortable width="200">
            </el-table-column>
            <el-table-column prop="film_info.quote" label="经典台词" show-overflow-tooltip sortable width="300">
            </el-table-column>
            ...
            <el-table-column v-if="role === 'admin'"  prop="enable" label="是否有效" show-overflow-tooltip width="100">
                <template slot-scope="scope">
                    <el-switch
                            ...
                            @change="changeReadingArticleInfoEnable(scope.$index, filmArticleList.docs)">
                    </el-switch>
                </template>
            </el-table-column>
            <el-table-column v-if="role === 'admin'"  prop="is_top" label="是否置顶" show-overflow-tooltip width="100">
                <template slot-scope="scope">
                    <el-switch
                            ...
                            active-color="#13ce66"
                            inactive-color="#ff4949"
                            @change="changeFilmArticleInfoIsTop(scope.$index, filmArticleList.docs)">
                    </el-switch>
                </template>
            </el-table-column>
            <el-table-column label="操作" width="100">
                <template slot-scope="scope">
                    <el-button class="edit-btn" size="mini" type="primary" plain round @click="editPost(scope.$index, filmArticleList.docs)"><i class="iconfont icon-caozuo-bianji"></i></el-button>
                    <el-button size="mini" v-if="scope.row.status === 0 && role === 'admin'" type="primary" plain  @click="toHandleStatus(scope.$index, filmArticleList.docs, 4)">待发布</el-button>
                    <el-button size="mini" v-if="scope.row.status === 2 && role === 'admin'" type="primary" plain  @click="toHandleStatus(scope.$index, filmArticleList.docs, 4)">待发布</el-button>
                    ...
                    <el-dropdown size="mini" v-if="scope.row.status === 1 && role === 'admin'" split-button type="info">
                        ...
                        <el-dropdown-menu slot="dropdown">
                            <el-dropdown-item ><span @click="toHandleStatus(scope.$index, filmArticleList.docs, 2)">审核通过</span></el-dropdown-item>
                            <el-dropdown-item><span @click="toHandleStatus(scope.$index, filmArticleList.docs, 3)">审核失败</span></el-dropdown-item>
                        </el-dropdown-menu>
                    </el-dropdown>
                    <el-button size="mini" v-if="scope.row.status === 0 && role === 'author'" type="primary" plain  @click="toHandleStatus(scope.$index, filmArticleList.docs, 1)">发布审核</el-button>
        ...
        <div class="pagination">
            <div v-if="filmArticleList.pageInfo">
                <el-pagination
                        @size-change="handleSizeChange"
                        @current-change="handleCurrentChange"
                        :current-page.sync="filmArticleList.pageInfo.current"
                        :page-size="filmArticleList.pageInfo.pageSize"
                        layout="total, prev, pager, next"
                        :total="filmArticleList.pageInfo.totalItems"
                >
                </el-pagination>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapActions({
                getFilmArticleList: types.GET_FILMARTICLELIST,
            }),
            ...
            changeFilmArticleInfoIsTop(index, rows){
                let rowData = rows[index];
                this.axios.updateFilmArticleInfo({
                    _id: rowData._id,
